...
function getWebpackConfig(options) {
  ...
  var styleLoader = { loader: "style-loader", options: { insertInto: "head", insertAt: { before: "[data-uu-app-styles-insert-before]" } } }; // into HEAD before given element (with fallback to the end of HEAD)
  ...
  var rules = [{
    oneOf: [ // use only first matched
      { test: /\.jsx?$/, use: [{ loader: "babel-loader", options: { plugins: ["react-hot-loader/babel"] } }, eslintLoader], parser: { import: false, system: false } },
      cssRule = { test: /\.css$/, use: [styleLoader, cssLoader, postCssLoader] },
      lessRule = { test: /\.less$/, use: [styleLoader, cssLoader, postCssLoader, "less-loader"] },
  ...
  if (opts.isProductionBuild) plugins.push(new webpack.optimize.ModuleConcatenationPlugin());
  ...
  webpackConfig.push({
    ...
    devtool: (opts.useSourceMaps && !opts.isProductionBuild) ? "cheap-module-eval-source-map" : false,
 ...
 * @param {*} depSettings Object containing settings of a single dependency.
 */
function replaceDependencyExpressions(depName, depSettings) {
  for (let depOption in depSettings) {
    ...
    depSettings[depOption] = depOptionValue.replace(/%([^%]*)%|\$\{([^}]*)\}/g, (m, ex1, ex2) => { // support %something% for backward compatibility
      ...
      let resolvedValue = pkg;
      let i = 0;
      while (i < parts.length && resolvedValue != null) {
        if (parts[i].match(/dependencies/i)) resolvedValue = getPackageJson(parts[++i], { version: "0.0.0" });
        else resolvedValue = resolvedValue[parts[i]];
        i++;
      }
