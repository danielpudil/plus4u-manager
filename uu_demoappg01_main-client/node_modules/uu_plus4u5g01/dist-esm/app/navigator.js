...
export const Navigator = createReactClass({
  ...
  _getContent(dtoOut) {
      ...
      for (let i = 0; i < navItems.data.itemList.length; i++) {
        ...
        if (this.props.small && this.state.menuHidden) {
          if (navItems.data.itemList[i].code === 'logout') {
            systemChildren.push(<UU5.Bricks.TouchIcon onClick={() => {
              this._stopPropagation = true;
    ...
    if (this.props.small && this.state.menuHidden) {
      ...
      output = [<UU5.Forms.TextIcon id={this.getId() + '-search'} className={this.getClassName('search')} icon={this.getDefault().send} onClick={this._search} ref_={input => this._input = input} inputAttrs={{
        onKeyDown: e => {
          if ((e.keyCode || e.which) === 13) {
            this._search();
          }
        }
      }} />, <UU5.Bricks.Div content={systemChildren} className={this.getClassName('system')} />, <UU5.Bricks.Line className={this.getClassName('divider')} colorSchema="grey-rich" size="s" />, <UU5.Bricks.Div content={children} className={this.getClassName('personal')} />];
    }
    return output;
  },
