...
let Calls = {
  ...
  loadNavigator: dtoIn => {
    Plus4U5.Common.Calls.getPersonalRoleUri(uri => Plus4U5.Common.Calls.getPropertyValue(uri + 'VAR.UES.COR.SYSTEM_NAVIGATOR', dtoIn.done, dtoIn.fail), dtoIn.fail);
  },
  ...
  loadNavigationBar: dtoIn => {
    ...
    if (dtoIn.data.navigationBarUri) {
      Plus4U5.Common.Calls.getPropertyValue(dtoIn.data.navigationBarUri, dtoOut => {
      ...
      let calls = ["UU.BEM/NAVIGATION_BAR_1/PORTAL", "UU.BEM/NAVIGATION_BAR_2/PORTAL"].map(code => {
        return new Promise((resolve, reject) => {
          Plus4U5.Common.Calls.getPropertyValue(acrUri + ":" + code, value => resolve(Plus4U5.Common.Tools.base64DecodeUnicode(value.dataHandler).data), reject);
        });
      });
