...
export const NavigationBarLoader = createReactClass({
  ...
  _getNavigationRow(navData) {
    ...
    if (leftGroup.length === 0 || rightGroup.length === 0) {
      return <UU5.Bricks.Div className={this.getClassName('center')}>
          {rightGroup.map((item, index) => {
          return this._getItem(item, index);
        })}
        </UU5.Bricks.Div>;
    } else {
      return <UU5.Bricks.Div className={this.getClassName('sides')}>
          <UU5.Bricks.Div>
  ...
  _getNavigationBar() {
    ...
    let welcomeText = navItems && navItems.welcomeText && <UU5.Bricks.Div className={this.getClassName('welcome') + ' ' + this.getClassName('center')}>
    ...
    return <UU5.Bricks.Div {...mainProps}>
        {welcomeText}
        {firstRow}
        {line}
        {secondRow}
