...
const RichImage = createReactClass({
  ...
  _getFrameDiv(shape, effect, children) {
    ...
    switch (effect) {
      case 'blur':
        r = <UU5.Bricks.Div mainAttrs={mainAttrs} className={this._getShape(this.props.shape).className + ' ' + this._getEffect(this.props.effect).className}>
              {shadowDiv}
              {children}
  ...
  render() {
    ...
    //<span>text</span>
    <img alt={this.getName()} src={src} ref={img => this._img = img} className={className} onLoad={() => this._onLoad()} />)
