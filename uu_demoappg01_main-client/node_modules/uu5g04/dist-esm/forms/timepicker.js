        ...
        if (typeof this.props.onChange === 'function') {
          ...
          if (this.props.validateOnChange) {
            ...
            if (opt.value === '') {
              ...
              if (!result.value || this.props.format === FORMAT_12 && result.value.match(/^\d{1,2}:?\d{0,2} ?[PpAa]?\.?[Mm]?\.?$/) || this.props.seconds && this.props.format === FORMAT_12 && result.value.match(/^\d{1,2}:?\d{0,2}:?\d{0,2} ?[PpAa]?\.?[Mm]?\.?$/) || this.props.format === FORMAT_24 && result.value.match(/^\d{1,2}:?\d{0,2}$/) || this.props.seconds && this.props.format === FORMAT_24 && result.value.match(/^\d{1,2}:?\d{0,2}:?\d{0,2}$/)) {
                this.setFeedback(result.feedback, result.message, result.value);
              }
            }
          }
  ...
  _formatTime(value, fill0) {
    ...
    if (value) {
      if (fill0) {
        ...
        this.props.seconds && (time += ':' + UU5.Common.Tools.rjust(value.seconds, 2, '0'));
      } else {
